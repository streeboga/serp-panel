...
| `groups` | какие группы проверок гонять; по умолчанию все |

Группы: `technical`, `meta`, `content`, `links`, `images`.
...
Каждая находка: `check` (код вроде `meta.title.missing`), `group`, `severity`
(`critical` / `warning` / `notice`), `message`, `value`, `expected`.
