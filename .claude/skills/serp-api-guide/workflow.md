...
**Заглушённые коды находок (муты).**

Часть находок для конкретного сайта ложна по построению. На eq.team
`http.analytics.missing` приходит на каждой странице: счётчик Метрики
подключается после ответа на cookie-баннер, а аудитор кликов не делает. Пока
такие коды считаются наравне с остальными, прогон показывает 1589 находок там,
где разбирать надо 26, и число перестают читать.

```
PATCH /projects/{projectId}/muted-codes
{"muted_codes": {"http.analytics.missing": "счётчик грузится после cookie-баннера"}}
```

Ключ — код НАХОДКИ (`content.nausea.academic`), а не код проверки
(`content.nausea`). Разница существенная: у одной проверки бывают и шумная
находка, и та, ради которой она заведена (`content.nausea.density`), поэтому
`check_codes` заглушке не замена. Причина обязательна и хранится рядом с кодом.
Пустой объект снимает политику. Текущая политика видна в `GET /projects/{id}`,
поле `muted_codes`.

Что делает заглушение:

* находка остаётся в `findings` с полями `muted: true` и `mute_reason` — «дефектов
  нет» и «мы перестали смотреть» должны различаться с одного взгляда;
* в `issues_critical`, `issues_warning`, `issues_notice` и в штраф оценки она не
  попадает, а считается отдельно в `issues_muted`;
* политика на момент запуска кладётся в сам прогон (`muted_codes` в `GET /audits/{id}`),
  поэтому старый прогон читается тем списком, по которому его считали.
...
**Выгрузки.** `GET /audits/{id}/export/{набор}` — четыре набора, которые просят в закупках:

| Набор | Что внутри |
|---|---|
| `pages` | все проверенные URL с кодами ответов, глубиной от главной и метриками |
| `meta` | Title, Description, заголовки, canonical, Schema.org |
| `broken` | битые ссылки и файлы с числом ссылающихся страниц |
| `findings` | все находки построчно, сначала уровень сайта |

CSV отдаётся с BOM и точкой с запятой: без первого русский Excel показывает кракозябры,
без второго вся строка падает в одну ячейку. Строки идут курсором, поэтому выгрузка
не упирается в память на больших сайтах.

**Структура сайта.** В прогоне появляются находки, которых постранично не бывает:
`site.structure.orphans` (на страницу не ведёт ни одна внутренняя ссылка),
`site.structure.unreachable` (ссылки есть, но от главной не дойти),
`site.structure.too_deep`, `site.anchors.spam`, `site.params.duplicates`,
`site.near_duplicate.title` и `.description`. У каждой страницы появляются
`depth` (кликов от главной) и `inbound_links`.

**Поведенческие данные.** Если у проекта задан `metrika_counter_id`, а у организации
есть токен Яндекса, в `metrics.behaviour` прогона приезжают посещаемость, отказы,
глубина, время и квантили загрузки, а страницы получают свои визиты и отказы.
Находка `behaviour.bounce` отмечает страницы входа с высокой долей отказов —
только там, где визитов хватает, чтобы судить.
