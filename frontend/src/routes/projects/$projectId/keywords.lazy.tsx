...
function KeywordsTable() {
        ...
        for (const dev of importDevices)
          for (const regionId of importRegionIds)
            for (const [clusterId, keywords] of byCluster)
              promises.push(importKeywords.mutateAsync({ keywords, engine: eng, device: dev, cluster_id: clusterId, region_id: regionId }))
  ...
  const handleBulkMove = useCallback(async () => {
    ...
    await Promise.all([...selectedIds].map(id => updateKeyword.mutateAsync({ id, cluster_id: Number(bulkMoveClusterId) })))
